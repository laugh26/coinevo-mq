...
void CoinevoMQ::proxy_batch(detail::Batch* batch) {
...
void CoinevoMQ::job(std::function<void()> f) {
...
void CoinevoMQ::proxy_schedule_reply_job(std::function<void()> f) {
    auto* b = new Batch<void>;
    b->add_job(std::move(f));
    batches.insert(b);
...
void CoinevoMQ::proxy_run_batch_jobs(std::queue<batch_job>& jobs, const int reserved, int& active, bool reply) {
...
void CoinevoMQ::proxy_timer(std::function<void()> job, std::chrono::milliseconds interval, bool squelch) {
    if (!timers)
        timers.reset(zmq_timers_new());

    int timer_id = zmq_timers_add(timers.get(),
            interval.count(),
            [](int timer_id, void* self) { static_cast<CoinevoMQ*>(self)->_queue_timer_job(timer_id); },
            this);
    if (timer_id == -1)
        throw zmq::error_t{};
...
void CoinevoMQ::proxy_timer(bt_list_consumer timer_data) {
...
void CoinevoMQ::_queue_timer_job(int timer_id) {
...
void CoinevoMQ::add_timer(std::function<void()> job, std::chrono::milliseconds interval, bool squelch) {
...
void CoinevoMQ::TimersDeleter::operator()(void* timers) { zmq_timers_destroy(&timers); }
